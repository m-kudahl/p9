...
app.get('/api/alerts', async (req, res) => { // We are making an asynchronous function, which means the application will still be responsive while performing tasks
  try {
...
app.put('/api/doorstatus', async (req, res) => {
  ...
  try {
    ...
    await fs.writeFile(doorStatusFilePath, JSON.stringify(updatedStatus, null, 2)); 
