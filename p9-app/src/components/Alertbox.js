...
export const handleAddAlert = async (type, message) => {
  // Takes parameters type and message, these are located in the HTML, and can be manually adjusted there
  try {
    const response = await fetch("http://localhost:8080/api/alerts", {
      // Attempts to POST a JSON string consisting of type and message
      method: "POST",
      headers: {
...
export default function Alertbox() {
  ...
  const handleAddAlertClick = async (type, message) => {
    // adds a new alert, with type and message depending on what button was clicked
    try {
  ...
  return (
    <div className="CenterBox">
      <div className="FixedButtonsContainer">
        ...
        <button onClick={() => handleAddAlertClick("error", "NEW ERROR")}>
          Add Error Alert
        </button>
        <button onClick={() => handleAddAlertClick("warning", "NEW WARNING")}>
          Add Warning Alert
        </button>
        <button onClick={() => handleAddAlertClick("info", "NEW INFO")}>
          Add Info Alert
        </button>
      </div>
